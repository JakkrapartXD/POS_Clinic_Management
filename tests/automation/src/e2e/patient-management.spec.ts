...
test.describe.serial('การทดสอบการจัดการผู้ป่วย E2E', () => {
  ...
  test('UAT-001: การป้องกันผู้ป่วยซ้ำ', async ({ page }) => {
    ...
    await page.fill('[data-testid="nickname-input"]', duplicatePatient.nickname);
    ...
    await page.fill('[data-testid="last-name-input"]', duplicatePatient.lastName);
    await page.fill('[data-testid="nickname-input"]', 'ชื่อเล่นอื่น');
  ...
  test('PatientService.create - กรอกข้อมูลไม่ครบ ระบบแจ้งเตือน "กรอกข้อมูลให้ครบถ้วน" และไม่บันทึกข้อมูล', async ({ page }) => {
    ...
    // ตรวจสอบว่า form ยังคงอยู่ (ไม่ submit)
