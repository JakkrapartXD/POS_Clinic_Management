...
test.describe.serial('การทดสอบการจัดการผู้ป่วย E2E', () => {
  ...
  test('PatientService.create - กรอกข้อมูลไม่ครบ ระบบแจ้งเตือน "กรอกข้อมูลให้ครบถ้วน" และไม่บันทึกข้อมูล', async ({ page }) => {
    // ไปที่หน้า patients
    await page.goto('/dashboard/patients');
    await page.waitForLoadState('networkidle');

    // คลิกปุ่ม "เพิ่มผู้ป่วยใหม่"
    await page.click('[data-testid="add-patient-button"]');
    await page.waitForSelector('[data-testid="patient-form"]');

    // กรอกข้อมูลบางส่วน (ไม่ครบ)
    await page.fill('[data-testid="first-name-input"]', 'ทดสอบ');
    // ไม่กรอก lastName
    await page.fill('[data-testid="phone-input"]', '0812345678');
    // ไม่กรอก email
    await page.fill('[data-testid="address-input"]', '123 ถนนทดสอบ');
    // ไม่กรอกข้อมูลอื่นๆ ที่จำเป็น

    // คลิกปุ่ม "บันทึก"
    await page.click('[data-testid="save-patient-button"]');

    // ตรวจสอบว่ามีการแจ้งเตือน "กรอกข้อมูลให้ครบถ้วน"
    await expect(page.locator('text=กรอกข้อมูลให้ครบถ้วน')).toBeVisible();
    
    // ตรวจสอบว่ายังอยู่ในหน้าเพิ่มผู้ป่วย (ไม่ redirect)
    await expect(page).toHaveURL(/.*\/dashboard\/patients.*add.*/);

    // ตรวจสอบว่าข้อมูลที่กรอกไว้ยังคงอยู่
    await expect(page.locator('[data-testid="first-name-input"]')).toHaveValue('ทดสอบ');
    await expect(page.locator('[data-testid="phone-input"]')).toHaveValue('0812345678');
    await expect(page.locator('[data-testid="address-input"]')).toHaveValue('123 ถนนทดสอบ');

    // ตรวจสอบว่าไม่มีข้อมูลใหม่ในตารางผู้ป่วย
    await page.goto('/dashboard/patients');
    await page.waitForLoadState('networkidle');
    
    // ตรวจสอบว่าไม่มีผู้ป่วยชื่อ "ทดสอบ" ในตาราง
    const patientTable = page.locator('[data-testid^="patient-row-"]');
    await expect(patientTable.locator('text=ทดสอบ')).not.toBeVisible();
  });
});
